(function (w){
	...
	View.prototype.bind = function(cmd,callback){
		...
		switch(cmd) {
			...
			break;
			case "makeListDroppable" :
			this.$container.find('.tasks').droppable({
				scope : 'a',
				...
				scroll : true,
				drop : function(e,ui){
					var target = e.target,dragged = ui.draggable;
					if(target != $(dragged).parent('ul')){
						$(target).append(dragged);
					}
				}
			});
			break;
			case "makeCardDraggable" :
			this.$container.find('.tasks li').draggable({
				scope : 'a',
				containment : self.$container,
				helper : 'clone',
